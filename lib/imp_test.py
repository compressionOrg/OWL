...
if __name__ == "__main__":
    ...
    #     z_scores[layer] = torch.abs((metric_conn[layer] - total_mean) / total_std)
    
    # layer_importance = {}
    # for layer in z_scores:
    #     layer_importance[layer] = torch.mean(z_scores[layer])
    ...
    # TODO:2.计算每一层mena值
    # for layer in metric_conn:
    #      metric_conn[layer] = torch.mean(metric_conn[layer]).item()

    # for layer in metric_node:
    #      metric_node[layer] = torch.mean(metric_node[layer]).item()
    ...
    # 选择node指标
    # sensitivities = torch.tensor([metric_node[x] for x in metric_node])
    
    # st()
    
    # total_mean = torch.mean(sensitivities)
    # total_std = torch.std(sensitivities)
    
    ## TODO:method1使用标准差来确定调节因子 beta
    ## ================A1=====================
    ...
    # # 计算每层的剪枝比例
    # scaled_ratio = beta * normalized_sensitivities
    
    # # 将剪枝范围映射到 [-x,x] 之间
    # min_ratio = torch.min(scaled_ratios)
    # max_ratio = torch.max(scaled_ratios)
    # scaled_ratios = (scaled_ratios - min_ratio) * (1/(max_ratio - min_ratio)*args.alpha*2)
    ## =================A1====================
